...
for i = 10:10
    ...
    for j=1:1
        ...
        for sigma_scale = 1:sigma_max
            ...
            sigma_price = 20*sigma_scale;
            P_pred = P_cur + normrnd(0,sigma_price,size(P_cur));
            sigma_demand = 20*sigma_scale;
            demand_pred = demand_cur + round(normrnd(0,sigma_demand,size(demand_cur)));
            for k = 1:iters
                [alg_cost] = BED(T,P_cur,P_pred,P_max,P_m,demand_cur,demand_pred,lambda,C);
                [online_cost] = BED(T,P_cur,P_pred,P_max,P_m,demand_cur,demand_pred,1,C);
                alg_costs(end +1) = alg_cost;
                online_costs(end+1) = online_cost;
            end
            avg_alg_cost = mean(alg_costs);
            ratio =avg_alg_cost/Cost_OPT;
            comp_ratios(end+1) = ratio;
            online_ratios(end+1) = mean(online_costs)/Cost_OPT;
        end
...
plot(linspace(20,20*200,200),comp_ratios,linspace(20,20*200,200),online_ratios)
title('CR vs. Prediction Error for Single Day')
xlabel('Prediction Error Parameter (standard deviation)')
ylabel('Competitive Ratio')
legend('\lambda = 0.5','\lambda = 1')